...
export type TokenListContextProps = {
  tokenInfos?: TokenRegistryInfo[];
};

const TokenListContext = React.createContext<TokenListContextProps>({});
  ...
  useEffect(() => {
    ...
    tokenListProvider.resolve().then((tokenListContainer) => {
      const filteredTokenListContainer =
        tokenListContainer?.filterByClusterSlug(cluster?.slug);
      let tokenInfos: TokenRegistryInfo[] | undefined =
        tokenListContainer !== filteredTokenListContainer
          ? filteredTokenListContainer?.getList()?.map((info) => {
              return {
                mintAddress: info.address,
                tokenName: info.name,
...
type TokenMintNameMap = Record<Mint, Record<Endpoint, TokenRegistryInfo>>;

const customTokenNamesByNetwork: TokenMintNameMap = JSON.parse(
...
export function getTokenInfo(
  ...
  let info: TokenRegistryInfo | undefined =
    customTokenNamesByNetwork?.[endpoint]?.[mint.toString()];
  let match: TokenRegistryInfo | undefined = tokenInfos?.find(
    (tokenInfo) => tokenInfo.mintAddress === mint.toString(),
  );

  if (match) {
    if (!info) {
      info = { ...match, icon: match.icon };
    }
    // The user has overridden a name locally.
    else {
      info = { ...match, ...info, icon: match.icon };
    }
  }
  return { ...info };
}
